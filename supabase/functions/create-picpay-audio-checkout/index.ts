...
// Preço por crédito de áudio (em centavos), igual à sua função do PayPal.
const AUDIO_CREDIT_PRICE = 10; 
...
serve(async (req) => {
  ...
  try {
    ...
    if (amount < 100) throw new Error("A quantidade mínima é 100 créditos");

    // Autenticação do usuário (igual à sua implementação original)
    const authHeader = req.headers.get("Authorization")!;
    const { data: { user } } = await supabaseClient.auth.getUser(authHeader.replace("Bearer ", ""));
    if (!user) throw new Error("Usuário não encontrado");

    const totalPrice = amount * AUDIO_CREDIT_PRICE;

    // Adapte o corpo da requisição (buyer) conforme a documentação da API PicPay
    // e os dados que você possui do seu usuário.
    const picpayPayload = {
      referenceId: `audio_${amount}_${user.id}_${new Date().getTime()}`,
