...
serve(async (req) => {
  ...
  try {
    ...
    const successUrl = `${origin}/chat-trial?credits_success=true&credits=${productData.credits}`;
    const cancelUrl = `${origin}/chat-trial?credits_canceled=true`;
      ...
      purchase_units: [
        {
          ...
          custom_id: `audio_credits_${user.id}_${Date.now()}`,
        },
      ],
      payment_source: {
        paypal: {
