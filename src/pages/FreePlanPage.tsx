...
const FreePlanPage = () => {
  ...
  useEffect(() => {
    ...
    // Plano gratuito (ID: 1)
    const plan = plans.find(p => p.id === 1);
  ...
  const handleSelectPlan = async (planId: number) => {
    ...
    try {
      console.log("Processando plano gratuito com ID:", planId);
      await selectPlan(planId);
      // Após selecionar o plano gratuito, redirecionar para o chat
      navigate('/chat');
    } catch (error: any) {
  ...
  return <SinglePlanCard plan={freePlan} onSelectPlan={handleSelectPlan} />;
};
