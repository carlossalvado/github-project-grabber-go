...
const UltimatePlanPage = () => {
  const { plans, loading, selectPlan } = useSubscription();
  const { user } = useAuth();
  const navigate = useNavigate();
  const [ultimatePlan, setUltimatePlan] = useState<any>(null);

  useEffect(() => {
    if (loading) return;
    
    // Plano ultimate/trial (ID: 4)
    const plan = plans.find(p => p.id === 4);
    ...
    setUltimatePlan(plan);
    
    // Salvar ID do plano no localStorage
    localStorage.setItem('selectedPlanId', '4');
  ...
  const handleSelectPlan = async (planId: number) => {
    ...
    try {
      console.log("Processando plano ultimate com ID:", planId);
  ...
  return <SinglePlanCard plan={ultimatePlan} onSelectPlan={handleSelectPlan} />;
};

export default UltimatePlanPage;
